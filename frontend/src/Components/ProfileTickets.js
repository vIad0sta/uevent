...
function ProfileTickets(props) {
    const handleVisibilityChange = async (event, checked, id) => {
        const response = await UserRequests.editBoughtTicket(id, { visible: checked });
    }

    return (
        <div className={'tickets-container'}>
            {props.tickets && props.tickets.map((ticket) => (
                <div className={'ticket-element'} key={ticket.id}>
                    <h3>Type: {ticket.Ticket.type}</h3>
                    <h4>Quantity: {ticket.quantity}</h4>
                    <span>Visible:<Switch defaultChecked={ticket.visible} color="warning" onChange={(event, checked) => handleVisibilityChange(event, checked, ticket.id)} /></span>
                </div>
            ))}
